...
const AllRecipes = ({ category, searchWord }) => {
  ...
  useEffect(() => {
    const fetchDummyData = async () => {
      try {
        const response = await axios.get("dummy.json"); // dummy.json 파일 요청
        const dummyData = response.data.recipes; // JSON 내부의 recipes 배열
        setDummyRecipes(dummyData); // dummyRecipes 상태에 저장
      } catch (error) {
